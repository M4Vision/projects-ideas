...
test.describe('ProTask gold-noir', () => {
  ...
  test('full walkthrough — covers all OpenAPI routes', async ({ page }) => {
    ...
    await page.goto('/protask/templates/gold-noir/index.html');

    // ── REGISTER ──
    await page.click('#login-toggle-link');
    await page.fill('#reg-name', 'E2E User');
    await page.fill('#login-email', 'e2e-gn@test.com');
  ...
  test('crée un board juste après inscription', async ({ page }) => {
    ...
    await page.fill('#login-email', 'nouveau-gn@test.com');
  ...
  test('inscription → créer board → ajouter cartes → dashboard → sélectionner board', async ({ page }) => {
    ...
    await page.fill('#login-email', 'createur-gn@test.com');
...
test.describe('ProTask — Invitations & CRUD (API directe)', () => {
  ...
  test('accepter invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      await window.demoApi.register({ name: 'U1', email: 'accept-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'accept-gn@test.com');
  ...
  test('refuser invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U2', email: 'decline-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'decline-gn@test.com');
  ...
  test('annuler invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U3', email: 'cancel-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'cancel-gn@test.com');
  ...
  test('retirer membre', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const r = await window.demoApi.register({ name: 'U4', email: 'remove-gn@test.com', password: 'pass1234' });
      const invitedUserId = r.user.id;
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'remove-gn@test.com');
    ...
    expect(result).not.toContain('remove-gn@test.com');
  });

  test('wrong user cannot respond', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U5', email: 'wrong-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'wrong-gn@test.com');
  ...
  test('déjà invité → erreur', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U6', email: 'dup-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      await window.demoApi.inviteMember(board.id, 'dup-gn@test.com');
      try { await window.demoApi.inviteMember(board.id, 'dup-gn@test.com'); return 'ok'; }
      catch (e) { return e.message; }
    });
  ...
  test('déjà membre → erreur', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U7', email: 'member-gn@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'member-gn@test.com');
      await window.demoApi.respondToInvitation(inv.id, 'accepted');
      try { await window.demoApi.inviteMember(board.id, 'member-gn@test.com'); return 'ok'; }
      catch (e) { return e.message; }
    });
  ...
  test('utilisateur inexistant → erreur', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      try { await window.demoApi.inviteMember(1, 'nobody-gn@test.com'); return 'ok'; }
      catch (e) { return e.message; }
    });
