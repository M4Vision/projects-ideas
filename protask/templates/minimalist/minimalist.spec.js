...
test.describe('ProTask minimalist', () => {
  ...
  test('full walkthrough — covers all OpenAPI routes', async ({ page }) => {
    ...
    await page.goto('/protask/templates/minimalist/index.html');
...
test.describe('ProTask — Invitations & CRUD (API directe)', () => {
  ...
  test('accepter invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      await window.demoApi.register({ name: 'U1', email: 'accept-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'accept-m@test.com');
  ...
  test('refuser invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U2', email: 'decline-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'decline-m@test.com');
  ...
  test('annuler invitation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U3', email: 'cancel-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'cancel-m@test.com');
  ...
  test('retirer membre', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const r = await window.demoApi.register({ name: 'U4', email: 'remove-m@test.com', password: 'pass1234' });
      const invitedUserId = r.user.id;
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'remove-m@test.com');
    ...
    expect(result).not.toContain('remove-m@test.com');
  });

  test('wrong user cannot respond', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U5', email: 'wrong-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'wrong-m@test.com');
  ...
  test('déjà invité → erreur', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U6', email: 'dup-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      await window.demoApi.inviteMember(board.id, 'dup-m@test.com');
      try { await window.demoApi.inviteMember(board.id, 'dup-m@test.com'); return 'ok'; }
      catch (e) { return e.message; }
    });
  ...
  test('déjà membre → erreur', async ({ page }) => {
    const result = await page.evaluate(async () => {
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      await window.demoApi.register({ name: 'U7', email: 'member-m@test.com', password: 'pass1234' });
      await window.demoApi.login({ email: 'alex@protask.dev', password: 'pass123' });
      const board = (await window.demoApi.getBoards())[0];
      const inv = await window.demoApi.inviteMember(board.id, 'member-m@test.com');
      await window.demoApi.respondToInvitation(inv.id, 'accepted');
      try { await window.demoApi.inviteMember(board.id, 'member-m@test.com'); return 'ok'; }
      catch (e) { return e.message; }
    });
  ...
  test('self-invite rejetée avec toast erreur', async ({ page }) => {
    ...
    if (!(await boardBtn.isVisible())) {
      await page.click('#dash-new-board-btn');
      await dialogConfirm(page, 'Invite Min Test');
      await page.waitForTimeout(300);
    } else {
  ...
  test('assigner un label à une carte via le modal UI', async ({ page }) => {
    ...
    await expect(adder).toBeVisible({ timeout: 3000 });
    await adder.click();
    await dialogConfirm(page, 'Carte Labels Min');
    await page.waitForTimeout(300);
    await page.evaluate(() => {
      const el = document.querySelector('.task-card');
      if (el) openCardModal(+el.dataset.cardId);
  ...
  test('assigner un label à une carte via API directe', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const card = await window.demoApi.createCard(cols[0].id, { title: 'Label Card Min', labels: [] });
